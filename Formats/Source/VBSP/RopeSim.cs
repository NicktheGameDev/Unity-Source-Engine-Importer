...
public class RopeSim : MonoBehaviour
{
    ...
    vp_RandomSpawner randomSpawner = null;
    [HideInInspector]
    ...
    public void ConfigureMovementSettings(float gravity, float strength, float damping, float noise, Vector3 windDirection, float windAmount)
    {
    ...
    public void ConfigureKeyframeSettings(float settleTime, float animationLength, float animationFPS, bool recordAnimation)
    {
        this.settleTime = settleTime;
        this.animationLength = animationLength;
        this.animationFPS = animationFPS;
        this.recordAnimation = recordAnimation;
    }

    // The existing methods like Awake, Update, etc. are unchanged

    void Awake()
    {
        generated = false;
        animator = GetComponent<Animator>();
        randomSpawner = GetComponent<vp_RandomSpawner>();
        if (start == null || end == null || start.position == end.position)
        {
            return;
        }

        if (start == null && randomSpawner.isSpawnfromRappel)
        {
            animator.SetTrigger("rappelLength");
            return;
        }
        ...
        if (recordAnimation)
        {
            ...
            clip = new AnimationClip() { hideFlags = HideFlags.DontSaveInEditor | HideFlags.DontSaveInBuild };
        ...
        if (recordAnimation)
        {
            AnimationClip clip = Resources.Load<AnimationClip>("Assets/" + transform.parent.gameObject.name + ".anim") as AnimationClip;
            if (clip)
            {
    ...
    void Update() {
        ...
        if ( recordAnimation && Time.time > animationLength+settleTime && !savedAnimation ) {
            ...
            clip.frameRate = 30;
			/*[DISABLED: runtime-only] AssetDatabase.CreateAsset(clip, "Assets/"+gameObject.transform.parent.gameObject.name+".anim");*/
