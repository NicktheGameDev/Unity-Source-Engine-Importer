...
namespace TF2Ls
{
    public class BlackMesaLsSettings : ScriptableObject
    {
        ...
        {
            var guids = AssetDatabase.FindAssets("t:" + nameof(BlackMesaLsSettings).ToLower());
        ...
        [SerializeField] string bmsPath;
        public string TFInstallPath => bmsPath;
        ...
        { 
            ...
            { 
                ...
                {
                    if (File.Exists(Path.Combine(bmsPath, ModelTexturerWindow.VTF_VPK_FILENAME))) return true;
                }
                return false;
            }
        }
        ...
        static BlackMesaLsSettings settings;
        public static BlackMesaLsSettings Settings
        {
            get
            {
                if (settings == null)
                {
                    var guids = AssetDatabase.FindAssets("t:" + nameof(BlackMesaLsSettings));
                    for (int i = 0; i < guids.Length; i++)
                    {
                        settings = AssetDatabase.LoadAssetAtPath<BlackMesaLsSettings>(AssetDatabase.GUIDToAssetPath(guids[i]));
                    }
                }
                return settings;
            }
        ...
        public static SettingsProvider CreateMyCustomSettingsProvider()
        {
            ...
            var provider = new SettingsProvider("Project/TF2Ls", SettingsScope.Project)
            {
                ...
                guiHandler = (searchContext) =>
                {
                    ...
                    var settings = BlackMesaLsSettings.SerializedObject;
                    ...
                    SerializedProperty bmsPath = settings.FindProperty(nameof(bmsPath));
                    ...
                    // Validate folder
                    EditorHelper.RenderSmartFolderProperty(new GUIContent("bms Path"), bmsPath, false, "Select the tf folder within your TF2 installation path");
                    if (unlockSystemObjects.boolValue)
                    {
                        EditorGUILayout.LabelField("Don't touch these files if you don't know what they do. " +
                            "Worst case scenario, you will have to re-import the package.", BlackMesaLsSettings.Settings.HelpTextStyle.ApplyBoldText());
                    }
                    ...
                    {
                        BlackMesaLsSettings.Settings.Reset();
                    }
                ...
                keywords = new HashSet<string>(new[] { "TF2BmsLs", "Team Fortress", "Package", "Presets", "Enums" ,"BlackMesa"})
            };
